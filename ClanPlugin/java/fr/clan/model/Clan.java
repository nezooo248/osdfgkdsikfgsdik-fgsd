...
public class Clan {
    ...
    public void demote(UUID uuid) { officers.remove(uuid); }

    public boolean canInvite(UUID uuid) { return isLeader(uuid) || isOfficer(uuid); }
    public boolean canKick(UUID uuid) { return isLeader(uuid) || isOfficer(uuid); }
    public boolean canManageRoles(UUID uuid) { return isLeader(uuid); }
}
